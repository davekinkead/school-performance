...
Simulation.prototype.graduate = function(graduation_rate) {
  ...
  return this.graduates = this.students.filter(function(student) {
    return Math.random() < graduation_rate;
  });
};
...
Simulation.prototype.rank_schools = function() {
  ...
  return ref = this.schools[0].score > this.schools[1].score ? [this.schools[0], this.schools[1]] : [this.schools[1], this.schools[0]], best = ref[0], worst = ref[1], ref;
};

Simulation.prototype.measure_schools = function() {
  return this.schools.map((function(_this) {
    return function(school) {
      var students, total;
      students = _this.students.filter(function(student) {
        return student.school.id === school.id;
      });
      total = students.map(function(student) {
...
display = (function(_this) {
  return function(id, params) {
    ...
    tick = function(simulation) {
      simulation.tick();
      render(simulation);
      return draw(canvas.selectAll("circle"));
    };
    draw = function(students) {
      students.transition().duration(1000).style("fill", function(d) {
...
window.onload = function() {
  ...
  return display('head-start-2', head_start_2);
};
